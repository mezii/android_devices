...
const getStringBeforeDot = (str) =>
  str ? (str.match(/^([^.]*)/) || [])[1] || "" : null;
...
router.get("/random", async (req, res) => {
  try {
    ...
    if (filteredDevices.length === 0) {
      return res
        .status(404)
        .json({ message: "No devices found in specified version range" });
    }

    // Select a random device
    const randomDevice =
      filteredDevices[Math.floor(Math.random() * filteredDevices.length)];
    // Find the corresponding version in the Version database
    const version = await Version.findOne({
      releaseType: randomDevice.VERSION,
    });

    if (!version) {
      return res
        .status(404)
        .json({ message: "No matching version found for the selected device" });
    }
    ...
    const HOST =
      "SWDD" +
      (Math.floor(Math.random() * (8000 - 3000 + 1)) + 3000).toString();
...
router.put("/update-version", async (req, res) => {
  try {
    await Device.updateMany({}, [
      {
        $set: {
          MAIN_VERSION: {
            $toInt: {
              $cond: {
                if: { $regexMatch: { input: "$VERSION", regex: "\\." } },
                then: { $arrayElemAt: [{ $split: ["$VERSION", "."] }, 0] },
                else: "$VERSION",
              },
            },
          },
    ...
    console.log("Updated all devices with MainVersion!");
  } catch (error) {
    console.log(error);
  }
  res.send("Success");
